...
(25/09/30 추가) 치지직 채팅 연동이 베타 기능으로 제공됩니다. 치지직 채널 ID를 알고 있다면 아래와 같이 `platform=chzzk` 쿼리를 사용해 접속할 수 있습니다.

```
https://lastorder.xyz/chatreader-kor/speech.html?platform=chzzk&channel=<치지직-채널-ID>
```

브라우저 상단의 채널 변경 버튼으로도 채널 ID를 다시 입력할 수 있으며, 라이브 중에 채팅 채널이 갱신되면 자동으로 재연결을 시도합니다. 치지직에서 채팅 명령으로 돌아가는 시스템 메시지는 봇 UI 알림으로 표시됩니다.
...
| vieweronly  | on, off         | 시청자 전용 채팅 모드를 활성화/비활성화 합니다. 활성화시 스트리머 제외 시청자의 채팅만 읽습니다.                    | !!tts vieweronly on       |
